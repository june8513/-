...
DEFAULT_MONITOR_DIR = os.path.join(BASE_DIR, 'auto_upload', 'supplier_data')
...
def run_monitor_supplier_data():
    print(f"--- Running Supplier Data Monitor ---")
    
    # Check AutoUploadConfig first
    config = AutoUploadConfig.objects.filter(upload_type='supplier_data', is_active=True).first()
        ...
        for file_path in files_to_check:
            ...
            try:
                ...
                if filename not in last_processed_timestamps or current_mtime > last_processed_timestamps[filename]:
                    ...
                    call_command('auto_upload_supplier_data', path=file_path)
