...
DEFAULT_MONITOR_DIR = os.path.join(BASE_DIR, 'auto_upload', 'shipping_customer')
...
def run_monitor_shipping_customer():
    print(f"--- Running Shipping & Customer Monitor ---")
    
    # Check AutoUploadConfig first
    config = AutoUploadConfig.objects.filter(upload_type='shipping_customer', is_active=True).first()
        ...
        for file_path in files_to_check:
            ...
            try:
                ...
                if filename not in last_processed_timestamps or current_mtime > last_processed_timestamps[filename]:
                    ...
                    call_command('auto_upload_shipping_customer', path=file_path)
