...
DEFAULT_MONITOR_DIR = os.path.join(BASE_DIR, 'auto_upload', 'semi_finished') 
...
def run_monitor_semi_finished():
    print(f"--- Running Semi-Finished Monitor ---")
    
    # Check AutoUploadConfig first
    config = AutoUploadConfig.objects.filter(upload_type='semi_finished', is_active=True).first()
        ...
        for file_path in files_to_check:
            ...
            try:
                ...
                if filename not in last_processed_timestamps or current_mtime > last_processed_timestamps[filename]:
                    ...
                    call_command('auto_upload_semi_finished', path=file_path)
                    print(f"Successfully processed {filename}.")
                    last_processed_timestamps[filename] = current_mtime
                    
                    # Update config status if applicable
                    if config:
                        config.last_run = django.utils.timezone.now()
    ...
    # Generic cleaning of timestamps for files in default dir
    # For single file config, we don't remove timestamp easily as path might change back and forth
    # But for simplicity, we keep the timestamp file locally
